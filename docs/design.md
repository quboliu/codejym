...
- **Storage**: PostgreSQL 存储用户/素材/会话元数据；本地磁盘 `data/uploads/<userId>/<assetId>` 存放真实源码。
- **Deployment**: 
...
|--------|------|-------------|
| `POST` | `/auth/signup` | 注册账号（邮箱、密码、昵称），返回 JWT。 |
| `POST` | `/auth/login` | 登录并返回 JWT。 |
| `GET` | `/auth/me` | 校验当前 token，返回用户信息。 |
...
所有 `/api/*` 业务接口均要求 `Authorization: Bearer <token>` 鉴权；token 采用 HS256 JWT，默认 30 天有效。

## 5. Data Model（当前实现）
- `users`：`id`、`email`、`name`、`password_hash`、`created_at`、`updated_at`。
- `assets`：`id`、`user_id`、`name`、`root_path`、`size_bytes`、`file_count`、`source_name`、`created_at`、`updated_at`。
- `typing_sessions`：`id`、`user_id`、`asset_id`、`rel_path`、`cursor`、`errors`、`duration_seconds`、`created_at`、`updated_at`。

PostgreSQL 负责所有结构化元数据；上传文件夹始终位于 `data/uploads/<userId>/<assetId>`，以便日后扩展为对象存储或云端挂载。命令行工具 `backend/cmd/migratejson` 可将旧版 JSON (`data/assets.json` / `data/sessions.json`) 导入数据库。
...
- 核心环境变量：
  - `ADDR`：HTTP 监听地址。
  - `DATA_DIR`：上传/缓存根目录（默认 `./data`）。
  - `FRONTEND_DIR`：前端静态资源目录。
  - `DATABASE_URL`：PostgreSQL 连接串（必填）。
  - `AUTH_SECRET`：HS256 签名用的随机密钥（必填）。
- 纯软件：`npm run build` 产出 `frontend/dist`，`go build` 后以 `DATABASE_URL` + `AUTH_SECRET` 运行。
- 容器：Dockerfile 多阶段构建，`docker compose` 同时启动 `postgres` + `codecopybook`，默认挂载 `./data` 及 PG 数据卷。
- `scripts/deploy.sh` 仍提供 `local` / `docker` 两种模式：本地模式会检查所需环境变量，Docker 模式调用 Compose 自动拉起数据库。
